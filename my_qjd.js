...
Date: 2021/7/4 上午10:02
抄于Author: Curtin 的jd_qjd.py
================Loon==============
[Script]
cron "10 0,7,23 * * *" script-path=jd_qjd.js
...
let UserAgent_diy = 'jdappiPhone10.0.413.7ca6eb91a888be488f194b9d9216cf711dd1b221anetwork/wifiADID/8679C062-A41A-4A25-88F1-50A7A3EEF34Amodel/iPhone8,1addressid/3723896896appBuild/167707jdSupportDarkMode/0Mozilla/5.0 (iPhone CPU iPhone OS 13_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148supportJDSHWK/1'
let helpInfo = {};
let helplist = {};
let help_diy ;
let helpToast = {};
...
!(async () => {
  ...
  if (process.env.QJD_HELPLIST ) {
    helplist = process.env.QJD_HELPLIST;
    helplist = helplist.split(",");
    help_diy = true;
    console.log(`\n======================自定义助力功能开启============================\n`)
    console.log(`\n======================${helplist}============================\n`)
  }
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr[i]) {
      ...
      await TotalBean();
      if (!$.isLogin) {
        console.log(`\n================账号${$.index} ${$.UserName}=未登录，请重新扫码登录==========================\n`)
      //  $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.nickName || $.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});
        continue;
      }

  if (help_diy == true ){
    for (let j in helplist ) {
    console.log(`\n================================【${j}】===================================\n`)
      $.groupCode = helpInfo[helplist[j]].groupCode
      $.shareCode = helpInfo[helplist[j]].shareCode
      $.activityId = helpInfo[helplist[j]].activityId
      let y = i +1+ "";
      if (helplist[j] == y ) {
        console.log(`${$.UserName}跳过助力自己`)
        continue;
      }
      console.log(`\n================账号${$.index} ${$.UserName}=开始为账号>>>${helplist[j]}<<=助力==========================\n`)
      await doHelp($.groupCode, $.shareCode, $.activityId)
      await $.wait(1000)
      if (helpToast === "您今日助力次数已达上限~" || helpToast === "活动太火爆啦~请稍后再试~" || helpToast === "京东服务器返回空数据") break;
    }
  }
    else{
    for (let j in helpInfo ) {
      console.log(`\n================================【${j}】===================================\n`)
      $.groupCode = helpInfo[j].groupCode
      $.shareCode = helpInfo[j].shareCode
      $.activityId = helpInfo[j].activityId
      let y = i +1 + "";
      if ( j == y ) {
        console.log(`${$.UserName}跳过助力自己`)
        continue
      }
      console.log(`\n================账号${$.index} ${$.UserName}=开始为账号>>>${j}<<=助力==========================\n`)
      await doHelp($.groupCode, $.shareCode, $.activityId)
      await $.wait(1000)
      if (helpToast === "您今日助力次数已达上限~" || helpToast === "活动太火爆啦~请稍后再试~" || helpToast === "京东服务器返回空数据") break;
      ...
      try {
        if (err) {
          ...
          if (data) {
           ...
           helpInfo[$.index] = {groupCode, shareCode, sumBeanNumStr, activityId}
  ...
  return new Promise(async resolve => {
    ...
    const options = {
       ...
       headers: {
        ...
        'Accept': '*/*',
        'Connection': 'keep-alive',
        'Referer': `https://h5.m.jd.com/rn/42yjy8na6pFsq1cx9MJQ5aTgu3kX/index.html?jklActivityId=116&source=SignSuccess&jklGroupCode=${groupCode}&ad_od=1&jklShareCode=${shareCode}`,
        'Accept-Encoding': 'gzip, deflate, br',
        'Host': 'api.m.jd.com',
        'User-Agent':  $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
        'Accept-Language': 'zh-cn'
