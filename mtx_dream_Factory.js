...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.index}${$.nickName || $.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});

        if ($.isNode()) {
          await notify.sendNotify(`${$.index}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
        }
        continue
      }
  ...
  $.tuaniddiy[0]= $.tuanIds[0] ;//by xp
  $.tuaniddiy[1]= $.tuanIds[1] ;//by xp
  for (let i = 0; i < cookiesArr.length; i++) {
   y = 0;//by xp
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
    ...
    if (i == 8 || i== 15){
      $.log(`开始领取账号1的开团奖励`);
      await tuanAward(tuanActiveId, $.tuanIds[0]);//isTuanLeader
      await $.wait(8000);
      $.log(`开始领取账号2的开团奖励`);
      await tuanAward(tuanActiveId, $.tuanIds[1]);//isTuanLeader
      await $.wait(8000);
    }

      // if ((cookiesArr && cookiesArr.length >= ($.tuanNum || 2)) && $.canHelp) {
      if ((cookiesArr && cookiesArr.length >= ($.tuanNum || 1)) && $.canHelp) {
        console.log(`\n================================账号${i+1}${$.UserName} 内部相互进团========================================\n`);//by xp
      //  for (let item of $.tuanIds) {
        for (let item of $.tuaniddiy) {

          if (!$.canHelp || y > 1) break;//by xp
          console.log(`\n账号${i+1}>>>>>>>${$.UserName}<<<<<<<<< 去参加账号${y+1} 的团>>>>ID为${item}<<<<`);
          y =y+1;
          //if (!$.canHelp) break;
          await JoinTuan(item);
          await $.wait(1000);
      ...
      if ($.canHelp && y == 100) await joinLeaderTuan();//参团
    }
  }
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                console.log(`\n【京东账号${$.index}（${$.index}）的${$.name}好友互助码】${data.user.encryptPin}`);
...
async function joinLeaderTuan() {
  ...
  $.authorTuanIds = $.tuanIds //by xp
  let x = 0 ;//by xp
  if ($.authorTuanIds && $.authorTuanIds.length) {
   // for (let tuanId of $.authorTuanIds) {
    for (let tuanId of $.tuaniddiy) {
      if (!tuanId) continue
      if (!$.canHelp) break;
      //console.log(`\n账号${$.UserName} 参加作者lxk0301的团 【${tuanId}】`);
      console.log(`\n账号${$.UserName}${$.index}>>>>>>>>>>> 参加ID为【${tuanId}】的团<<<<<<<<<`);//by xp
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if ($.shareCodesArr[$.index - 1]) {
      ...
      console.log(`由于您第${$.index}个京东账号${$.index}未提供shareCode,将采纳本脚本自带的助力码\n`)
    ...
    console.log(`第${$.index}${$.index}个京东账号将要助力的好友${JSON.stringify($.newShareCodes)}`)
    resolve();
  })
}
